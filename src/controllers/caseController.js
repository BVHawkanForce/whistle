...
exports.getCase = async (req, res) => {
    ...
    try {
        // Determine if the code is for a reporter or a handler
        const isHandler = code.length === 32; // Assuming handler codes are longer
        const codeColumn = isHandler ? 'handler_code' : 'reporter_code';

        // Fetch the report
        const reportQuery = `SELECT * FROM reports WHERE ${codeColumn} = $1`;
